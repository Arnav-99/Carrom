...
def gameMode_mouseReleased(app,event):
    modeDemo.loadSprites(app,app.striker.direction)

def setFriction(app):
    app.friction=app.getUserInput("Enter friction coefficient:")
    app.friction=float(app.friction)

def gameMode_timerFired(app): 
    app.sliders={"force":(app.forceSliderX,312),"friction":(app.frictionSliderX,312),"spin":(app.spinSliderX,437),"direction":(app.directionSliderX,562)}
    if not app.gameOver:
        if app.pointerPulsation%(int(5*(50000/app.striker.force)))==0:
            app.spriteCounter = (1 + app.spriteCounter) % len(app.sprites)
        app.pointerPulsation+=1
        if app.playButton.isButtonPressed() and app.paused:
...
def checkIfTurnEnded(app):
    ...
    if not app.striker.vel==0:
        ...
        app.startX,app.startY=0,0
        origScore=currPlayer.currScore
        ...
        if not currPlayer.hasStruckQueen:
...
def checkForAndHandleCollision(app,isElastic=False):
    ...
    for j in range(len(app.pegsList)):
        for k in range(len(app.pegsList)):
            ...
            and not (app.pegsList[j].inGutter or app.pegsList[k].inGutter):
                ...
                direction=math.tan(0.25*((y1-y2)/(x1-x2)))*180/math.pi
                ...
                #The code shall recursively check thrice for collision at each calling,
                #to ensure that the speed of the peg does not cause the collision to not be triggered
                #return
...
def helpMode_redrawAll(app,canvas):
    canvas.create_text(app.width/2,app.height/2,text="HELP MODE",font="Arial 26 bold")
